...
class Calendario:
    ...
    def popola_calendario(self):
        ...
        while i < self.totale_giorni:
            ...
            dip_meno_turni = list(sorted(self.persone, key=lambda x: x.numero_turni))
            ...
            if i == 0:
                ...
                while len(g.mattina) < 2:
                    x += 1
                    p = random.choice(dip_meno_turni)
                    if p not in g.mattina and p not in self.giorni[i-1].notte and p not in self.giorni[i-2].notte and p.numero_turni < self.turni_persona:
                        g.mattina.append(p)
                        p.numero_turni += 1
                    if x > 1000000:
                        x = 0
                ...
                while len(g.pomeriggio) < 2:
                    x += 1
                    p = random.choice(dip_meno_turni)
                    if p not in g.mattina and p not in g.pomeriggio and p not in self.giorni[i-1].notte and p not in self.giorni[i-2].notte and p.numero_turni < self.turni_persona:
                        g.pomeriggio.append(p)
                        p.numero_turni += 1
                    if x > 1000000:
                        x = 0
                ...
                while len(g.notte) < 2:
                    x += 1
                    p = random.choice(dip_meno_turni)
                    if p not in g.mattina and p not in g.pomeriggio and p not in g.notte and p not in self.giorni[i-1].notte and p not in self.giorni[i-2].notte and p.numero_turni < (self.turni_persona - 1):
                        g.notte.append(p)
                        p.numero_turni += 2
                    if x > 1000000:
                        x = 0
        ...
        pp = sorted(self.persone, key=lambda x: x.numero_turni)
        while pp[-1].numero_turni == self.turni_persona:
            print('Elimino: ', pp[-1].nomecognome(), pp[-1].numero_turni)
            pp.pop()
        for p in pp:
            x = 0
            while p.numero_turni < self.turni_persona:
                x += 1
                gg = list(self.giorni)
                i = random.randrange(len(gg))
                g = gg[i]
                if len(g.mattina) <= 2 and p not in g.mattina and p not in g.pomeriggio and p not in g.notte and p not in gg[i-1].notte:
                    self.giorni[i].mattina.append(p)
                    p.numero_turni += 1
                elif len(g.pomeriggio) <= 2 and p not in g.mattina and p not in g.pomeriggio and p not in g.notte and p not in gg[i-1].notte:
                    self.giorni[i].pomeriggio.append(p)
                    p.numero_turni += 1
                if x > 1000000:
                    x = 0
                    return False
    ...
    def salva_calendario(self):
        with open('calendario.csv', 'w') as file:
            ...
            for t in self.giorni:
                ...
                m3 = ''
                if len(t.mattina) == 3:
                    m3 = t.mattina[2].nomecognome()
                p1 = t.pomeriggio[0].nomecognome()
                p2 = t.pomeriggio[1].nomecognome()
                p3 = ''
                if len(t.pomeriggio) == 3:
                    p3 = t.pomeriggio[2].nomecognome()
                n1 = t.notte[0].nomecognome()
                n2 = t.notte[1].nomecognome()
                riga1 = [m1, p1, n1]
                riga2 = [m2, p2, n2]
                riga3 = []
                if len(t.mattina) == 3 or len(t.pomeriggio) == 3:
                    riga3.append(m3)
                    riga3.append(p3)
                writer.writerow(d)
                writer.writerow(intestazione)
                writer.writerow(riga1)
                writer.writerow(riga2)
                if len(t.mattina) == 3 or len(t.pomeriggio) == 3:
                    writer.writerow(riga3)
